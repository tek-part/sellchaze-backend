...
class ThemeSettingsTranslationTest extends TestCase
{
    ...
    public function test_resolver_returns_flat_settings_for_the_locale_plus_settings_i18n(): void
    {
        ...
        $theme = Theme::query()->where('key', 'naseem')->firstOrFail();
        $install = $service->install($store, $theme, app(ThemeRegistry::class)->resolveThemeVersion($theme));
        $service->updateSettings($install, ['announcement_text' => ['en' => 'Free shipping', 'ar' => 'شحن مجاني']]);
        $service->publish($store, $install->fresh());
        $service->activate($store, $install->fresh());

        $stored = StoreTheme::query()->find($install->id);
        $this->assertSame(['en' => 'Free shipping', 'ar' => 'شحن مجاني'], $stored->settings['announcement_text']);

        $resolver = app(ThemeResolver::class);
        $ar = $resolver->resolve($store, 'ar');
        $this->assertSame('شحن مجاني', $ar['settings']['announcement_text']);
        $this->assertSame(['en' => 'Free shipping', 'ar' => 'شحن مجاني'], $ar['settings_i18n']['announcement_text']);
        $this->assertSame('Free shipping', $resolver->resolve($store, 'en')['settings']['announcement_text']);

        // Public payloads carry flat scalars for the request locale.
        $this->getJson('http://nike.sellchase.com/api/v1/storefront?lang=ar')
            ->assertOk()
            ->assertJsonPath('theme.settings.announcement_text', 'شحن مجاني')
            ->assertJsonPath('theme.settings_i18n.announcement_text.en', 'Free shipping');
        $this->getJson('http://nike.sellchase.com/api/v1/storefront/context?template=home&lang=en')
            ->assertOk()
            ->assertJsonPath('theme.settings.announcement_text', 'Free shipping')
            ->assertJsonPath('locale.current', 'en');
    }

    public function test_manifests_flag_text_settings_as_translatable(): void
    {
        foreach (['naseem', 'bazaar', 'sahra', 'fresh', 'techno'] as $key) {
            $manifest = json_decode(file_get_contents(resource_path("themes/storefront/{$key}.json")), true);
            $field = collect($manifest['settings_schema'])->flatMap(fn ($g) => $g['fields'])->firstWhere('id', 'announcement_text');
            $this->assertTrue($field['translatable'] ?? false, "{$key}.announcement_text");
        }
    }
}
