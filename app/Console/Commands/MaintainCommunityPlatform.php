    ...
    public function handle(FeedCache $cache): int
    {
        ...
        Post::query()->published()->where('published_at', '>=', now()->subDays(45))->select(['id', 'likes_count', 'comments_count', 'shares_count', 'published_at'])->chunkById(250, function ($posts) use (&$ranked) {
            ...
            foreach ($posts as $post) {
                $signal = $events->get($post->id);
                $ageHours = max(1, $post->published_at?->diffInHours(now()) ?? 1);
                ...
                $post->updateQuietly(['ranking_score' => round($quality / pow($ageHours + 2, .42), 4)]);
                $ranked++;
            }
        });
        ...
        Hashtag::query()->chunkById(200, function ($tags) {
            foreach ($tags as $tag) {
                $recent = $tag->posts()->where('status', 'published')->where('lifecycle_status', 'published')->where('published_at', '>=', now()->subDays(7))->count();
                $tag->update(['posts_count' => $tag->posts()->where('status', 'published')->where('lifecycle_status', 'published')->count(), 'trend_score' => $recent * 3]);
            }
        });
