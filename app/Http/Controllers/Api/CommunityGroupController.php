...
class CommunityGroupController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        if ($request->boolean('joined')) {
            $q->whereHas('members', fn ($m) => $m->where('users.id', $viewerId)->where('community_group_memberships.status', 'active'));
        }
        if ($request->filled('q')) {
            $q->where(fn ($x) => $x->where('name', 'like', '%'.$request->string('q').'%')->orWhere('description', 'like', '%'.$request->string('q').'%'));
        }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (! empty($data['organization_id'])) {
            abort_unless($request->user()->organizationMemberships()->where('organization_id', $data['organization_id'])->where('status', 'active')->exists(), 403);
        }
    ...
    public function leave(Request $request, CommunityGroup $group): JsonResponse
    {
        ...
        DB::transaction(function () use ($request, $group) {
            ...
            if ($active && $group->members_count > 1) {
