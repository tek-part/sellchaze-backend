<x-default-layout :title="$title" :breadcrumb="$breadcrumb">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col">
                            <h4 class="card-title">{{ __('My deals out') }}</h4>
                            <p class="text-muted mb-0 small">{{ __('Accepted deals as customer — suppliers you dealt with.') }}</p>
                        </div>
                        <div class="col-auto">
                            @if(!$quotations->isEmpty())
                                <button type="button" class="btn btn-light border" id="crm-deals-out-excel-btn" title="{{ __('Export Excel') }}">
                ...
                <div class="card-body pt-0">
                    ...
                    <div class="table-responsive">
                        <table class="table mb-0 crm-datatable" data-export-name="deals-out" id="kt_table_deals_out" data-dt-hide-buttons-ui="1" data-dt-order='[[7,"desc"]]'>
                            <thead class="table-light">
                                <tr>
                                    <th class="ps-4">{{ __('Supplier') }}</th>
                                    <th>{{ __('Order') }}</th>
                                    <th>{{ __('Price') }}</th>
                            ...
                            <tbody>
                                ...
                                @php $sup = $quotation->supplierUser; @endphp
                                <tr>
                                    <td class="ps-4">
                                        @if($sup && $sup->profile)
                                            <a href="{{ route('profile.show', $sup->profile->username) }}" target="_blank" class="fw-medium text-primary">{{ $sup->name }}</a>
                                        @else
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var tableEl = document.getElementById('kt_table_deals_out');
            var excelBtn = document.getElementById('crm-deals-out-excel-btn');
