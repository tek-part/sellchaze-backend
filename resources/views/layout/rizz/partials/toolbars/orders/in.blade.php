...
<div class="card-header border-0 pt-6">
    <div class="card-title">
        <form method="GET" action="{{ route('orders.in') }}" id="kt_orders_in_filter_form" class="d-flex flex-wrap align-items-center gap-3">
            ...
            <input type="date" name="date_from" class="form-control form-control-solid w-150px" value="{{ ($filters ?? [])['date_from'] ?? '' }}" />
            <input type="date" name="date_to" class="form-control form-control-solid w-150px" value="{{ ($filters ?? [])['date_to'] ?? '' }}" />
            <button type="submit" class="btn btn-light-primary">{{ __('Apply') }}</button>
            <a href="{{ route('orders.in') }}" class="btn btn-light">{{ __('Reset') }}</a>
        </form>
    </div>
